package basisFx.appCore.grid;

import basisFx.appCore.elements.TableWrapper;
import javafx.geometry.HPos;
import javafx.geometry.Insets;
import javafx.geometry.VPos;
import javafx.scene.control.Button;

public class GridOrganizationButtomRightMiddleBigSingleTable extends GridOrganization{
    private TableWrapper tableWrapper;
    private Button buttonAdd;
    private Button buttonDel;

    public GridOrganizationButtomRightMiddleBigSingleTable(TableWrapper tableWrapper) {
        this.tableWrapper=tableWrapper;
        buttonAdd =buttonFactory.addRowButton(tableWrapper);
        buttonDel = buttonFactory.deleteRowButton(tableWrapper);

    }

    @Override
    public void organize(   ) {

        bindHeight(tableWrapper);
        parentGridWrapper.addSpanNode(
                parentGridWrapper.label.getElement(),
                0, 0, 2, 1, HPos.LEFT, VPos.TOP, insets);
        parentGridWrapper.addSpanNode(
                buttonAdd,
                1, 1, 1, 1, HPos.CENTER, VPos.TOP, insets);
        parentGridWrapper.addSpanNode(
                buttonDel,
                1, 2, 1, 1, HPos.CENTER, VPos.TOP, insets);
        parentGridWrapper.addSpanNode(
                tableWrapper.getElement(),
                0, 1, 1, 2, HPos.CENTER, VPos.TOP, insets);

    }
}
